...
use rs_merkle::{
    MerkleTree,
    algorithms::Sha256 as MerkleSha256
};
...
impl AuditingService {
    ...
    pub async fn anchor_audit_logs(&self) -> Result<()> {
        ...
        let merkle_root = merkle_tree.root().ok_or_else(|| anyhow::anyhow!("Failed to get Merkle root"))?;

        println!("Calculated Merkle root: {:?}, batch size: {}", merkle_root, logs.len());

        // Call hedera_service to anchor the root
        let transaction_record = self.hedera_service.anchor_log_batch(merkle_root, logs.len() as u64).await?;
        println!("Successfully anchored log batch. Transaction ID: {:?}", transaction_record.transaction_id);

        // Mark logs as anchored in the database
        let anchor_batch_id = ObjectId::new();
        self.db.mark_logs_as_anchored(&log_ids, anchor_batch_id).await?;
        println!("Successfully marked {} logs as anchored with batch ID: {}", log_ids.len(), anchor_batch_id);
